...
import jiang.device_upgrade.config.WebSocketServer;
 ...
 */
@ServerEndpoint(value = "/dispatcher/{userId}/{userName}")
@Component
@Scope("prototype")
    ...
    public void sendToUser(String sockMsg) {
        ...
        try {
            if (WebSocketServer.getSocks().get(destUserno) != null) {
                Message msg = new Message(this.userId, this.userName, sendMessage, GlobalMessageEnum.USER.getCode());
